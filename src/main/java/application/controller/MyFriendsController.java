...
import application.services.MyFriendsServices;
...
public class MyFriendsController {
    ...
    public ResponseEntity<Void> bloquear(@PathVariable(name="email") String email, @RequestBody String friend,
        ...
        if (user == null){
            ...
            List<MyFriends> bloqueos = userAmic.getMyFriends();
            bloqueos.removeIf(myFriends -> myFriends.getEstado()!=Relacion.BLOQUEADO);
            if(bloqueos.size() >= 3){
                usuarioServices.deleteUsuarioByEmail(friend);
            }
